...
def download_omsagent(options, pythonCommand):
    ...
    if(returncode == 0):
        ...
        if(returncode != 0):
            ...
            if(pythonCommand == "python3"):
                cmdToInvokePerformedRequiredConfigChecks = ["sudo", "su", "omsagent", "-c", "python3 /opt/microsoft/omsconfig/Scripts/python3/PerformRequiredConfigurationChecks.py"]
            else:
                cmdToInvokePerformedRequiredConfigChecks = ["sudo", "su", "omsagent", "-c", "python2 /opt/microsoft/omsconfig/Scripts/PerformRequiredConfigurationChecks.py"]
            run_command(cmdToInvokePerformedRequiredConfigChecks)
            return True
    else:
        return False

def register_woker(options, pythonCommand):
    ...
    if(worker_present):
        cmdToRegisterUserHW = ["sudo", pythonCommand, "/opt/microsoft/omsconfig/modules/nxOMSAutomationWorker/DSCResources/MSFT_nxOMSAutomationWorkerResource/automationworker/scripts/onboarding.py", "--register", "-w", workspaceId,"-k", automationSharedKey, "-g",hybridGroupName,"-e", automationEndpoint]

        returncode, _, stderr = run_command(cmdToRegisterUserHW)
        if(returncode == 0):
            cmdToTurnOffSignValidation = ["sudo", pythonCommand ,"/opt/microsoft/omsconfig/modules/nxOMSAutomationWorker/DSCResources/MSFT_nxOMSAutomationWorkerResource/automationworker/scripts/require_runbook_signature.py", "--false", workspaceId]
...
def main():
    ...
    if(download_omsagent(options, pythonCommand)):
        register_woker(options, pythonCommand)
